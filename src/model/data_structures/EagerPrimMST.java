...
//        assert check(G);
    ...
    public EagerPrimMST(EdgeWeightedGraph G, int s) {
    	
    	this.s = s;
    	
    	if( s > G.V() )
    		throw new IllegalArgumentException("the id of the vertex s is greater than the size of the graph");
        ...
        int v = s;
        for (int i = 0; i < G.V(); i++){      // run from each vertex to find
        	if( v == G.V() )
        		v = 0;
...
//        assert check(G);
    ...
    public Edge[] edgesTo(){
    	Edge[] mst = new Edge[edgeTo.length];
        for (int v = 0; v < edgeTo.length; v++) {
            Edge e = edgeTo[v];
            if (e != null) {
                mst[v] = e;
    ...
    public CheapestPath<Edge> networkPath( MaxPQ<Integer> vertices ){
    	
    	CheapestPath<Edge> cp = new CheapestPath<Edge>();
    	cp.setFirstElement(s);
    	
    	while( !vertices.isEmpty() ){
    		int v = vertices.delMax();
    		int x = v;
    		for (Edge e = edgeTo[v]; e != null; e = edgeTo[x]) {
	            cp.addPathStep(e, e.weight1());
	            x = e.other(x);
	        }
    ...
    /**
     * Return the number of elements in the MST
     * @return size. Element in the MST
     */
    public int size(){
    	return edgeTo.length-1;
    }
